...
Uses the detected container engine (docker or podman) to build `cvo260917-deps:latest` from `Dockerfile.deps`. The build args are read from `[tool.docker-versions]` in `pyproject.toml`.
