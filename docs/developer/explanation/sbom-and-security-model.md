...
`./workflow.cmd secure.sbom-extract --write` generates a [CycloneDX](https://cyclonedx.org/) 1.7 bill of materials and writes it to `src/cvo260917/sbom.cdx.json`. Because that path is inside the package data tree, `uv build` automatically ships the SBOM **inside the wheel** — a downstream consumer unpacks the wheel and finds `cvo260917/sbom.cdx.json` alongside the Python modules.
...
- A downstream consumer can extract the SBOM from the wheel with `unzip -p <wheel> cvo260917/sbom.cdx.json` or `importlib.resources` — no separate artefact to track.
...
```bash
gh attestation verify cvo260917-<version>-py3-none-any.whl --repo <owner>/<repo>
```
